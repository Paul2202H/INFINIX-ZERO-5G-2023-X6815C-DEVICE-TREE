...
void vendor_load_properties()
{
    ...
    for (const string &prop : prop_partitions)
    {
        ...
        property_override(string("ro.product.") + prop + string("name"), "X6815C-GL");
        property_override(string("ro.product.") + prop + string("device"), "INFINIX-X6815C");
        property_override(string("ro.product.") + prop + string("model"), "INFINIX X6815C");
